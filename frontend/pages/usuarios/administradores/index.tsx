...
export default function Coordenadores() {
  ...
  async function fetchUsers(_page, _search, _status) {
    ...
    const options = {
      url: `${process.env.api}/users?type=admin&page=${_page}&limit=15&search=${_search}${parseUserActiveParam(_status)}`,
      method: "GET",
      headers: {
  ...
  return (
    <>
      <Head>
        <title>Administradores - {process.env.title}</title>
      </Head>
