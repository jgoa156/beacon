...
export default function FormAddBranch({
  ...
  const operation = branchProp == null ? "Adicionar" : "Editar";
  ...
  useEffect(() => {
    if (branchProp != null) {
      setName(branchProp.name);
      setCode(branchProp.code.toString());
      setPeriods(branchProp.periods.toString());
      setMinWorkload(branchProp.minWorkload.toString());

      const _education = branchProp.activityGroups.find((group) => group.name === "Ensino");
      const _research = branchProp.activityGroups.find((group) => group.name === "Pesquisa");
      const _extension = branchProp.activityGroups.find((group) => group.name === "Extensão");
  ...
  async function fetchAddBranch(data, isEdit = false) {
    setFetching(true);

    const options = {
      url: `${process.env.api}/branches${isEdit ? `/${branchProp.id}` : ""}`,
  ...
  return (
    <Content>
      ...
      <div style={{ width: "100%" }}>
        ...
        <Button style={{ marginTop: 15 }} onClick={(e) => handleAddBranch(e)}>
          {fetching ? (
