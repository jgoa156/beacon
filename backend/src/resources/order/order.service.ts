...
export class OrderService {
	...
	async downloadOrder(orderId: number, res: any) {
		try {
			...
			const filePath = `./public/files/orders/${order.file}`;
			...
			res.setHeader(
				"Content-disposition",
				"attachment; filename=" + order.file,
			);
	...
	async updateSearchHash(id: number) {
		...
		searchHash.push(order.workload);
